...
$(function() {
	...
	systemJQGridList$.jqGrid({
		...
		height : height,
		// 列显示名称，是一个数组对象
		colNames : ['id','英文姓','英文名','出生日期','证件种类','证件号码','证件有效期','邀请人姓名','邀请人身份证号','联系方式'],
		// 列显示模板
		...
		colModel : [
					...
					{ name : 'inviter',index : 'yqrxm',width : 10,align : "center"},
					{ name : 'inviterIdCard',index : 'yqrfzh',width : 13,align : "center"},
					{ name : 'phone',index : 'lxfs',width : 10,align : "center"}
				]
		...
		onClickButton : function() {
			var url = basePath+"qiantai/jsp/addmykaqzysq.jsp";
			url = encodeURI(url);
	...
	.navGrid('#systemJQGridPager', {
		...
		var realId = id.id;
		var url = basePath+"qiantai/jsp/editmykaqzysq.jsp?id="+realId;
		url = encodeURI(url);
		window.location.href = url;
	   } else {
